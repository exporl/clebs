...
defineTest(clebsAddTestTarget) {
    ...
    for(alldir, CLEBS_TREES) {
        ...
        for(testdir, testdirs) {
            ...
            eval(testxml_$${target}.commands = -$${xvfb} $${DESTDIR}/$${target} -xunitxml | $${DESTDIR}/qunitxmlfix > $${target}-results.xml)
            eval(testinstalledxml_$${target}.commands = -$${xvfb} $${BINDIR}/$${target} -xunitxml | $${DESTDIR}/qunitxmlfix > $${target}-results.xml)
